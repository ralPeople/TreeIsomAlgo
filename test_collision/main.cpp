...
void solve2() {
	...
	MOD = 1'000'000'000'000 + 39;
	ll ans = 0;
	ll res = 0;
	ll val = INF;
	vector<vll>d(n+1,vll(n+1));
	for(ll X = 1; X <= MOD - 1; X++) {
		pows = vll(n+1,1);
		fi(1, n) pows[i] = mult(X,pows[i - 1]); 
		bool f = 1;
		fi(1, n) {
			ll hsh = 0;
			ll c = 0;
			...
			fj(i, n) {
				c++;
				hsh = add(hsh, mult(a[j], pows[c])); 
				if(hsh == 0) {
					ans += f; // another root for Polynom(i,j) is X
					f = 0;
					res++;
					d[i][j]++;
				}
